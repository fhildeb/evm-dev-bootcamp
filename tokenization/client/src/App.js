...
class App extends Component {
  state = { loaded: false, KYCAddress: "0x123", tokenSaleAddress: "", userTokenBalance: 0 };
  ...
  componentDidMount = async () => {
    try {
        ...
        this.kycManagerInstance = await new this.web3.eth.Contract(
          KYCManager.abi,
          KYCManager.networks[this.networkId] && KYCManager.networks[this.networkId].address,
        );

        // Set web3, accounts, and contract to the state, and then proceed with an
        // example of interacting with the contract's methods.
        this.listenToTokenTransfer();
        this.setState({ loaded:true, tokenSaleAddress: TokenSale.networks[this.networkId].address }, this.updateUserTokenBalance);
    } catch (error) {
  ...
  handleKYCSubmit = async () => {
    ...
    await this.kycManagerInstance.methods.setKYCCompleted(KYCAddress).send({from: this.accounts[0]});
  ...
  updateUserTokenBalance = async() => {
    let getTokenBalance = await this.tokenInstance.methods.balanceOf(this.accounts[0]).call();
    this.setState({userTokenBalance: getTokenBalance});
  }

  listenToTokenTransfer = async() => {
    this.tokenInstance.events.Transfer({to: this.accounts[0]}).on("data", this.updateUserTokenBalance);
  }
