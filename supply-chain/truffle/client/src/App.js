...
class App extends Component {
  ...
  handleSubmit = async () => {
    ...
    console.log(result);
    alert("Send "+cost+" wei to "+result.events.ItemStatusChanged.returnValues._address);
  };

  listenToPaymentEvent = () => {
    let self = this;
    this.itemManager.events.ItemStatusChanged().on("data", async function(event) {
      if(event.returnValues._status == 1) {
        let item = await self.itemManager.methods.items(event.returnValues._index).call();
        console.log(item);
